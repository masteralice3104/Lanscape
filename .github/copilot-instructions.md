...
- ヘッダ必須：`ip,segments,name,auto_name,mac,os_guess,ssh_banner,smb_banner,cert_cn,cert_san,http_server`（旧: `ip,user_space,manual_name`）
...
- `auto_name`：lookupService → mdns → ping -a → rdns → netbios → http-fp(本文/ヘッダ) → http(タイトル/Server) → cert → ssh → 空
- `cert_san`：TLS証明書SAN
- `http_server`：HTTPヘッダ
